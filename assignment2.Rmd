...
```
# Task 3
The score statistic can, like the likelihood ratio statistic, be generalized to the case with a nuisance parameter 𝜂. The generalized score statistic is

𝑇𝑆(𝜽0)=𝑆(𝜽0,𝜼̂ 𝑀𝐿(𝜽0))𝑇𝐼(𝜽0,𝜼̂ 𝑀𝐿(𝜽0))−1𝑆(𝜽0,𝜼̂ 𝑀𝐿(𝜽0))
with an asymptotic 𝜒2(𝑞) distribution (notation following the textbook chapter 5.5). An advantage of this statistic is thet the ML-estimate only needs to be computed under the null hypothesis. Compute the ML estimate of 𝜼=(𝜃𝐴𝑙𝑑𝑒𝑟,𝜃𝑈𝑡𝑏𝑖𝑙𝑑𝑎𝑟𝑒) under 𝐻0:𝜽=(𝜃𝑖𝑛𝑡𝑒𝑟𝑐𝑒𝑝𝑡,𝜃𝐾𝑜𝑛)=(0,0) and use this to determine a 𝑃
...
If we want to maximize 𝜼↦𝐿(𝜽,𝜼) for a fix 𝜽≠0 the function NR needs to be modified. Instead of doing so, we use R’s glm function with a so called offset. An offset is a variable 𝑜𝑖 that is added to the linear component 𝑥𝑖𝜃without a coefficient. For the logistic regression with offset 𝑜𝑖we then get 𝑝(𝑥𝑖)=(1+exp(−𝑥𝑖𝜃+𝑜𝑖))
...
```
#Task 4:
Compute the profile likelihood (textbook definition 5.4) for parameter 𝜃𝐾𝑜𝑛
, 𝐿𝑝(𝜃𝐾𝑜𝑛)
, on a suitable grid of parameter values. Use these to graph 𝐿𝑝
 together with the corresponding estimated likelihood (textbook definition 5.5). In order to determine 𝜂̂ 𝑀𝐿(𝜃𝐾𝑜𝑛)
 you may for example use the glm.fit function with an extra offset as in t
```{r}
...
which gives estimates of the other coefficients when 𝜃𝐾𝑜𝑛=0.5
 (as an example value). Decide a 95% confidence interval based on the profile likelihood visually from the figure by drawing a horizontal line at a suitable level (c.f. Figure 5.3b in the textbook). The choice of level should be motivated and the result compared with the corresponding Wald interval.
